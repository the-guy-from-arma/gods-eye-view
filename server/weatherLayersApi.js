...
export function createWeatherLayersApi(options = {}) {
  ...
  async function refreshRadar() {
    const response = await fetch(NOAA_RADAR_EXPORT, {
      ...
      headers: { Accept: 'image/png', 'User-Agent': 'ThunderLinkGodsEye/0.3.17 (+https://github.com/the-guy-from-arma/gods-eye-view)' },
    });
  ...
  async function refreshAlerts() {
    const response = await fetch(NWS_ALERTS_URL, {
      ...
      headers: {
        ...
        'User-Agent': env.NWS_USER_AGENT || 'ThunderLinkGodsEye/0.3.17 (https://github.com/the-guy-from-arma/gods-eye-view)',
      },
    });
