...
| **U.S. state 511 / DOT traveler-information catalogs** | Public traffic-camera locations and provider-published snapshots/streams for Arizona, Florida, Georgia, North Carolina, Utah, Nevada, Louisiana, Oregon, Michigan, and Indiana | Provider traveler-information terms and notices: [AZ511](https://az511.gov/), [FL511](https://fl511.com/), [511GA](https://511ga.org/), [DriveNC](https://drivenc.gov/), [UDOT Traffic](https://udottraffic.utah.gov/), [NV Roads](https://www.nvroads.com/), [511LA](https://511la.org/), [ODOT TripCheck](https://www.tripcheck.com/), [MDOT MiDrive](https://mdotjboss.state.mi.us/MiDrive/), [INDOT TrafficWise](https://511in.org/) | Name the applicable state 511/DOT provider; provider terms apply |
...
- **Additional U.S. state 511/DOT cameras.** `/api/cctv/sources` also loads the complete currently published camera catalogs for Arizona (ADOT), Florida (FDOT), Georgia (GDOT), North Carolina (NCDOT), Utah (UDOT), Nevada (NDOT), Louisiana (LADOTD), Oregon (ODOT TripCheck), Michigan (MDOT MiDrive), and Indiana (INDOT TrafficWise). The integration uses the same 15-minute server catalog cache and provider-isolated refresh policy as WSDOT. Provider status and accepted counts are returned in the endpoint's `catalogs` field. Public snapshots remain snapshots; HLS is registered only when the catalog says it is ungated. No login/session bypass is attempted. `CCTV_STATE_511_ENABLED=0` disables this group; `CCTV_STATE_511_PROVIDERS` accepts a comma-separated subset of `az,fl,ga,nc,ut,nv,la,or,mi,in`. Illinois Travel Midwest was audited but is not presented as a working catalog because its current response contains no camera rows.
