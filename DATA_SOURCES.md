...
- **Additional U.S. state 511/DOT cameras.** `/api/cctv/sources` loads complete currently published camera catalogs across Arizona, Florida, Georgia, North Carolina, South Carolina, Tennessee, Alabama, Utah, Nevada, Louisiana, Oregon, Michigan, Indiana, Ohio, Pennsylvania, New York, Virginia, New Jersey, Connecticut, Vermont, and Massachusetts. The integration uses a 15-minute server catalog cache and provider-isolated refresh policy; provider status and accepted counts are returned in the endpoint's `catalogs` field. Public snapshots remain snapshots and HLS is registered only when a provider publishes an ungated stream. Connecticut and Vermont use the public anti-forgery session issued by their own camera-list pages; no user login is performed. Alabama and Massachusetts expose placement metadata only because their provider policies require separate permission for third-party imagery. `CCTV_STATE_511_ENABLED=0` disables the group. Legacy `CCTV_STATE_511_PROVIDERS` values are treated as additions so old Railway configuration cannot silently suppress newly shipped providers; use `CCTV_STATE_511_EXCLUDE_PROVIDERS` for an explicit comma-separated exclusion list. Illinois Travel Midwest was audited but is not presented as a working catalog because its current response contains no camera rows.
