...
- **Steam / BattleMetrics Gaming Data.** `/api/gaming/games` and `/api/gaming/servers` are read-only backend routes. With server-only `STEAM_WEB_API_KEY`, Steam supplies per-game global active-player totals and public dedicated-server rows; servers are grouped at the eight coarse regions reported by Steam and the heatmap weights those regional points by connected server population. It never requests or returns Steam IDs for individual players, profile locations, friends, ownership, or player history, and it does not claim a player's physical location matches a server region. `STEAM_GAMING_APP_IDS` can restrict the bounded default game catalog. BattleMetrics remains an optional provider through server-only `BATTLEMETRICS_API_TOKEN`; its normalized responses retain five-minute caching and a 24-hour last-good fallback. Provider keys never enter the browser.
