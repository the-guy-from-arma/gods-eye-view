...
| **Broadcastify Feed Catalog** | Licensed law-enforcement feed metadata and provider page links | Broadcastify Developer Program terms; no audio proxying or redistribution | "Broadcastify" plus a link to the provider feed page |
...
- **Broadcastify Feed Catalog.** `/api/broadcastify/feeds` uses the server-only `BROADCASTIFY_API_KEY` to request the licensed Public Safety catalog, retains law-enforcement and combined-dispatch records, strips upstream audio URLs and credential material, and returns only normalized metadata plus canonical Broadcastify feed-page links. The five-minute cache is bounded by a 24-hour last-good fallback. The browser does not proxy, relay, record, or redistribute Broadcastify audio.
