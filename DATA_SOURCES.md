...
| **NASA EONET v3** | Keyless, continuously updated natural-event locations for the Live Global Events layer | NASA open-data terms and source-specific metadata disclaimers | "NASA EONET" plus the linked originating event source |
| **GDACS** | Keyless global disaster alerts and severity for the Live Global Events layer | GDACS data/API terms; event links remain authoritative | "GDACS" (European Commission / United Nations cooperation framework) |
| **NOAA / National Weather Service MRMS and Alerts API** | Five-minute radar reflectivity overlay and active georeferenced warning polygons | U.S. federal government public data; NOAA/NWS service terms and disclaimers apply | "NOAA / National Weather Service" |
...
| **OpenStreetMap (Nominatim)** | Reverse-geocoded Local Info labels and fallback address/place/intersection search | ODbL 1.0 + Nominatim usage policy | "© OpenStreetMap contributors" |
...
| **GDELT Project DOC 2.0** | Fail-soft cockpit headlines plus the Global News Reports index for CNN, Fox News, and ABC News incident reporting | [GDELT Terms of Use](https://www.gdeltproject.org/about.html#termsofuse): unrestricted academic/commercial/governmental dataset use, with citation and link required; linked articles retain publisher terms | "GDELT Project" plus each article's linked publisher/domain |
| **CNN Latest RSS** | Publisher fallback headlines for Global News Reports | CNN terms; linked publisher content is not included in the app | "CNN" plus a direct article link |
| **Fox News Latest RSS** | Publisher fallback headlines for Global News Reports | [Fox RSS Terms](https://www.foxnews.com/story/foxnews-com-rss-feeds): personal, non-commercial use only; direct article link required; no feed modification or associated advertising | "FOX NEWS" plus a direct article link |
| **ABC News Top Stories RSS** | Publisher fallback headlines for Global News Reports | [ABC News RSS information](https://abcnews.go.com/Site/page/rss-feeds-3520115); linked publisher content retains ABC's terms | "ABC NEWS" plus a direct article link |
...
| **Washington 511 / WSDOT ArcGIS Open Data** | Statewide public traffic-camera locations + registered snapshot URLs | [WSDOT Traveler Information / ArcGIS service](https://data.wsdot.wa.gov/arcgis/rest/services/TravelInformation/TravelInfoCamerasWeather/FeatureServer/0) | "Washington 511 · Washington State Department of Transportation" |
| **U.S. state 511 / DOT traveler-information catalogs** | Public traffic-camera locations and provider-published snapshots/streams across 21 supported states | Provider traveler-information terms and notices: [AZ511](https://az511.gov/), [FL511](https://fl511.com/), [511GA](https://511ga.org/), [DriveNC](https://drivenc.gov/), [511SC](https://www.511sc.org/), [Tennessee SmartWay](https://smartway.tn.gov/), [ALGO Traffic](https://www.algotraffic.com/), [UDOT Traffic](https://udottraffic.utah.gov/), [NV Roads](https://www.nvroads.com/), [511LA](https://511la.org/), [ODOT TripCheck](https://www.tripcheck.com/), [MDOT MiDrive](https://mdotjboss.state.mi.us/MiDrive/), [INDOT TrafficWise](https://511in.org/), [OHGO](https://ohgo.com/), [511PA](https://www.511pa.com/), [511NY](https://www.511ny.org/), [Virginia 511](https://511.vdot.virginia.gov/), [511NJ](https://511nj.org/), [CTroads](https://www.ctroads.org/), [New England 511](https://newengland511.org/), and [Massachusetts 511](https://mass511.com/) | Name the applicable state 511/DOT provider; provider terms apply. Alabama and Massachusetts are placement-only pending authorized media access. |
...
| **Broadcastify Feed Catalog** | Licensed Public Safety, Special Event, Disaster Event, and top-listener feed metadata plus provider page links | Broadcastify Developer Program terms; no audio proxying or redistribution | "Broadcastify" plus a link to the provider feed page |
| **Steam Web API** | Per-game global activity totals and public dedicated-server population grouped into Steam's eight coarse server regions | [Valve Steamworks Web API documentation](https://partner.steamgames.com/doc/webapi); server-list access uses a server-only Steam Web API key | "Game activity and regional dedicated-server data provided by Steam" |
| **BattleMetrics API** | Optional fallback multiplayer game/server directory, server status, approximate hosting location, and population | [BattleMetrics API documentation](https://www.battlemetrics.com/developers/documentation) and provider terms; a qualifying server-only bearer token may be required | "Gaming server data provided by BattleMetrics" |
...
- **Location search.** `/api/location/search` keeps the Google credential on the server and tries Google Geocoding, then Google Places Text Search. If both are unavailable or return no match, it serializes keyless Nominatim lookups at no more than one request per second; intersection queries resolve each named OSM road's returned geometry and compute their crossing. Only OpenStreetMap fallback results are cached (24-hour in-memory LRU); Google results remain live-only and return `Cache-Control: no-store`. The unified result supports street addresses, named places, intersections, cities, towns, and larger administrative regions.
- **Live Global Events.** `/api/live-events` merges the public NASA EONET v3 open-event GeoJSON feed with GDACS's recent global disaster GeoJSON. No API key is required. Successful results are normalized, bounded to 500 events, cached for five minutes, and may be served stale for up to 24 hours during a source outage. A partial response remains honest about which source failed. Globe cards link to the originating EONET/GDACS report and indicate source, category, time, country, severity, and GDACS alert level. These are situational-awareness leads, not independent verification or instructions for emergency response.
- **Live radar and warnings.** `/api/weather/radar.png` caches NOAA/NWS MRMS base-reflectivity imagery for five minutes and renders it as a translucent globe overlay. NOAA describes the service as updating approximately every five minutes and covering the United States, Canada, Alaska, the Caribbean, Guam, and Hawaii; transparent areas outside that footprint are not represented as missing storms. `/api/weather/alerts` retrieves active NWS GeoJSON alerts every two minutes, removes long free-text fields, and renders only alerts that include Polygon or MultiPolygon geometry. The layer reports total versus mapped alerts because some valid NWS products reference zones without embedding geometry. Both routes may serve a last-good response for up to one hour during a source outage. These layers are weather awareness aids, not official emergency instructions; users must follow the linked/local NWS authority.
- **Global News Reports.** `/api/news-events` makes one combined, rate-limited GDELT DOC query for recent mass-shooting, active-shooter, school-shooting, and mass-casualty reporting from CNN, Fox News, and ABC News, while each publisher's own RSS headlines provide a fail-soft fallback during index throttling. It retains only allow-listed publisher domains, direct publisher links, and titles with a conservative location hint. A bounded set is geocoded server-side with the configured `GOOGLE_MAPS_API_KEY`, cached in memory for 15 minutes (with a six-hour last-good fallback), and rendered as approximate `REPORTED · UNVERIFIED` markers. The app does not copy article bodies, summaries, or images. Publisher reports and automated location matches are leads, not independent confirmation, emergency instructions, or evidence of current danger. Fox's RSS terms limit that fallback to personal, non-commercial deployments; commercial operators must disable/replace it or obtain permission.
...
- **Washington 511 cameras.** `/api/cctv/sources` requests WSDOT's keyless public `TravelInfoCamerasWeather` ArcGIS FeatureServer layer, validates its Pacific Northwest service-footprint point geometry and HTTPS image URLs, and merges the complete current catalog—including WSDOT's small Portland-metro TripCheck partner set—into the CCTV layer. The camera list is cached for 15 minutes; frame media is requested only for selected/visible cards through the existing bounded CCTV frame scheduler. `CCTV_WSDOT_511_ENABLED=0` disables this provider and `CCTV_WSDOT_511_MAX_SOURCES` can lower its 2,000-record safety ceiling. The old OSIRIS-referenced `data.wsdot.wa.gov/log/public/cameras.json` URL is not used because WSDOT now returns 404 there.
- **Additional U.S. state 511/DOT cameras.** `/api/cctv/sources` loads current public camera catalogs across Arizona, Arkansas, Colorado, Connecticut, Florida, Georgia, Illinois, Indiana, Iowa, Kansas, Louisiana, Massachusetts, Michigan, Minnesota, Missouri, Nebraska, Nevada, New Jersey, New Mexico, New York, North Carolina, Ohio, Oklahoma, Oregon, Pennsylvania, South Carolina, South Dakota, Tennessee, Utah, Vermont, Virginia, and Wisconsin. California and Washington are loaded by their dedicated open-data adapters. The integration uses a 15-minute server catalog cache and provider-isolated refresh policy; provider status and accepted counts are returned in the endpoint's `catalogs` field. A provider that returns an incomplete page set serves its valid partial catalog with a degraded status instead of blanking already-available cameras. Public snapshots remain snapshots and HLS is registered only when a provider publishes an ungated stream. Wisconsin, Connecticut, and Vermont use the public anti-forgery session issued by their own camera-list pages; no user login is performed. Oklahoma, Alabama, and Massachusetts expose placement metadata only where media is not published for third-party playback. Travel Midwest/IDOT images are displayed unmodified and refreshed no more than once every five minutes under the provider's reuse policy. `CCTV_STATE_511_ENABLED=0` disables the group. Legacy `CCTV_STATE_511_PROVIDERS` values are treated as additions so old Railway configuration cannot silently suppress newly shipped providers; use `CCTV_STATE_511_EXCLUDE_PROVIDERS` for an explicit comma-separated exclusion list. Operators can also enable or hide each loaded state independently from the CCTV panel; that preference remains local to the browser.
...
- **Broadcastify Feed Catalog.** `/api/broadcastify/feeds` uses the server-only `BROADCASTIFY_API_KEY` to request the licensed Public Safety, Special Event, Disaster Event, and top-listener catalogs. It retains ordinary law-enforcement/combined-dispatch records plus provider-classified event feeds, strips upstream audio URLs and credential material, and returns only normalized metadata plus canonical Broadcastify feed-page links. Event genres are labeled `SPECIAL EVENT FEED` or `DISASTER EVENT FEED`; top-listener entries are labeled `ELEVATED LISTENER ACTIVITY`. None of those labels independently confirms an emergency. The five-minute cache is bounded by a 24-hour last-good fallback. The browser does not proxy, relay, record, or redistribute Broadcastify audio.
- **Steam / BattleMetrics Gaming Data.** `/api/gaming/games` and `/api/gaming/servers` are read-only backend routes. With server-only `STEAM_WEB_API_KEY`, Steam supplies per-game global active-player totals and the current public dedicated-server catalog window; servers are grouped at the eight coarse regions reported by Steam. The client distributes a bounded set of deterministic white/green visualization dots from that regional server share and the selected games' global concurrency. Dots are not player records or observed locations. The integration never requests or returns individual Steam IDs, profile locations, friends, ownership, or player history, and it does not claim a player's physical location matches a server region. `STEAM_GAMING_APP_IDS` can restrict the bounded default game catalog. BattleMetrics remains an optional provider through server-only `BATTLEMETRICS_API_TOKEN`; its normalized responses retain five-minute caching and a 24-hour last-good fallback. Provider keys never enter the browser.
...
> If you use ThunderLink God's Eye commercially, delete `src/data/local_data/telegeography_submarine_cables/` (or obtain a commercial license from TeleGeography). It is one self-contained folder; the rest of the app runs without it.
