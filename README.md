...
ThunderLink uses Railway Postgres for approved operator accounts, secure sessions, public-safety jurisdiction metadata, owner-controlled access, and a sanitized activity log. Attach Postgres so Railway injects `DATABASE_URL`, then configure `OWNER_EMAIL` and `OWNER_PASSWORD`. The account matching `OWNER_EMAIL` receives owner access and can open the Owner Dashboard.
...
Production starts at the account lock and does not initialize Cesium or any live data layer until an approved session exists. Resend and email verification are not used. New registrations are stored as pending requests by default. From the Owner Dashboard, the owner can approve or deny each request, or turn **Autopilot** on to approve future valid registrations immediately. Turning Autopilot off restores manual approval. The first owner can alternatively be bootstrapped with `OWNER_SETUP_TOKEN`: choose **Create Account**, enter `OWNER_EMAIL`, a new password, and that one-time code, then remove the variable after the owner is claimed.
