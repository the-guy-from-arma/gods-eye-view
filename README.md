...
| 📹 **CCTV Mesh** | 43,500+ public camera views and placements projected *into* the 3D space — Austin · London · Washington plus supported state 511/DOT catalogs and metro feeds across 36 states. Every loaded state has its own on/off control in the CCTV panel. Oklahoma, Alabama, and Massachusetts are placement-only where public media is not available; Illinois imagery follows Travel Midwest's five-minute reuse cadence. Positions are published; poses are estimated priors **you calibrate by dragging a gizmo on the camera itself** | Public transport APIs | 🟢 |
