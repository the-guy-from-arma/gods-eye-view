...
ThunderLink uses Railway Postgres for approved operator accounts, secure sessions, public-safety jurisdiction metadata, owner-controlled access, and a sanitized activity log. Attach Postgres so Railway injects `DATABASE_URL`, then configure `OWNER_EMAIL` and `OWNER_PASSWORD`. The account matching `OWNER_EMAIL` receives owner access and can open the full-page Owner Command Center at `/owner.html`.

For a Railway-managed owner login, set `OWNER_PASSWORD` to a unique 12–256 character server-only variable. Signing in with `OWNER_EMAIL` and that password creates or refreshes the approved owner record. Never prefix it with `VITE_`, expose it to client code, or commit it to Git.

Production starts at the account lock and does not initialize Cesium or any live data layer until an approved session exists. Resend and email verification are not used. New registrations are stored as pending requests by default. From Owner Command, the owner can approve or deny requests, place individual accounts under a security lock (which revokes all of their sessions), or turn **Autopilot** on to approve future valid registrations immediately. Site-wide controls switch the public console among **Systems Online**, **Maintenance Mode**, **Feed Disconnected**, and **Restricted Mode**; every non-online state presents a red satellite-link status screen while preserving owner recovery access. Turning Autopilot off restores manual approval. The first owner can alternatively be bootstrapped with `OWNER_SETUP_TOKEN`: choose **Create Account**, enter `OWNER_EMAIL`, a new password, and that one-time code, then remove the variable after the owner is claimed.
...
| 📹 **CCTV Mesh** | 31,700+ public camera placements projected *into* the 3D space — Austin · London · Washington plus complete supported state 511/DOT catalogs across California, Arizona, Florida, Georgia, North Carolina, South Carolina, Tennessee, Alabama, Utah, Nevada, Louisiana, Oregon, Michigan, Indiana, Ohio, Pennsylvania, New York, Virginia, New Jersey, Connecticut, Vermont, and Massachusetts. Alabama and Massachusetts are placement-only under their current provider media restrictions. Positions are published; poses are estimated priors **you calibrate by dragging a gizmo on the camera itself** | Public transport APIs | 🟢 |
