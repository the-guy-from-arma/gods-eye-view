...
test('owner access opens a dedicated full-page command center', () => {
  ...
  assert.match(owner, /TBSGE-KERNEL-030\.029/);
  assert.match(owner, /id="vehicle-analytics"/);
  assert.match(owner, /License plates, faces, occupants, unique vehicle identifiers/);
...
test('public console exposes ThunderLink OS identity without removing map attribution', () => {
  ...
  assert.match(index, /VERSION <strong>0\.3\.28<\/strong>/);
  assert.match(index, /KERNEL <strong>TBSGE-KERNEL-030\.029<\/strong>/);
